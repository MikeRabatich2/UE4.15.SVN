...
float UCharacterMovementComponent::MIN_FLOOR_DIST = 1.9f;
float UCharacterMovementComponent::MAX_FLOOR_DIST = 2.4f;
	...
	: Super(ObjectInitializer)
{
	...
	CustomGravity = FVector(0.f, 0.f, -980.f);
}
...
FVector UCharacterMovementComponent::GetBestDirectionOffActor(AActor* BaseActor) const
{
	...
	return GetCapsuleRotation().RotateVector(FVector(FMath::Cos(RandAngle), FMath::Sin(RandAngle), 0.5f).GetSafeNormal());
}
...
void UCharacterMovementComponent::SetDefaultMovementMode()
{
	...
	else if ( !CharacterOwner || MovementMode != DefaultLandMovementMode )
	{
		...
		//const float SavedVelocityUp = Velocity | CapsuleUp;
		...
		{
			// Prevent temporary walking state from zeroing Z velocity.
			//Velocity = FVector::VectorPlaneProject(Velocity, CapsuleUp) + CapsuleUp * SavedVelocityUp;
		...
		else
		{
			...
			if (bRotationChanged && !bIgnoreBaseRotation)
			{
				...
				if (PawnOldQuat.Equals(FinalQuat, 1e-6f))
				{
					...
					// @todo: This assumes only Yaw is used. Is this a valid assumption now that we have any directional gravity? This is the only reason FaceRotation() is used above really, aside from being a virtual hook.
...
void UCharacterMovementComponent::Crouch(bool bClientSimulation)
{
	...
	if( !bClientSimulation )
	{
		...
		if (ClampedCrouchedHalfHeight > OldUnscaledHalfHeight)
		{
			...
			const bool bEncroached = GetWorld()->OverlapBlockingTestByChannel(UpdatedComponent->GetComponentLocation() + CapsuleDown * ScaledHalfHeightAdjust, GetCapsuleRotation(),
...
void UCharacterMovementComponent::UnCrouch(bool bClientSimulation)
{
	...
	if( !bClientSimulation )
	{
		...
		{
			// Expand in place
			bEncroached = GetWorld()->OverlapBlockingTestByChannel(PawnLocation, CapsuleRotation, CollisionChannel, StandingCapsuleShape, CapsuleParams, ResponseParam);
			...
			if (bEncroached)
			{
				...
				if (ScaledHalfHeightAdjust > 0.f)
				{
					...
					const FVector Down = CapsuleDown * TraceDist;

					FHitResult Hit(1.f);
					const FCollisionShape ShortCapsuleShape = GetPawnCapsuleCollisionShape(SHRINK_HeightCustom, ShrinkHalfHeight);
					const bool bBlockingHit = GetWorld()->SweepSingleByChannel(Hit, PawnLocation, PawnLocation + Down, CapsuleRotation, CollisionChannel, ShortCapsuleShape, CapsuleParams);
					...
					else
					{
						...
						bEncroached = GetWorld()->OverlapBlockingTestByChannel(NewLoc, CapsuleRotation, CollisionChannel, StandingCapsuleShape, CapsuleParams, ResponseParam);
						if (!bEncroached)
						{
							// Intentionally not using MoveUpdatedComponent, where a horizontal plane constraint would prevent the base of the capsule from staying at the same spot.
		...
		else
		{
			...
			bEncroached = GetWorld()->OverlapBlockingTestByChannel(StandingLocation, CapsuleRotation, CollisionChannel, StandingCapsuleShape, CapsuleParams, ResponseParam);
...
void UCharacterMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed)
{
	...
	if (IsMovingOnGround())
	{
		RequestedVelocity = FVector::VectorPlaneProject(RequestedVelocity, GetCapsuleAxisZ());
	}
}
...
float UCharacterMovementComponent::GetMaxJumpHeight() const
{
	...
	if (FMath::Abs(GravityMagnitude) > KINDA_SMALL_NUMBER)
	{
		return FMath::Square(JumpZVelocity) / (-2.f * GravityMagnitude);
	}
	else
	{
		return 0.f;
...
void UCharacterMovementComponent::ApplyRootMotionToVelocity(float deltaTime)
{
	...
	const float UpVelocityMag = AppliedVelocityDelta | GetGravityDirection();
	if (bAppliedRootMotion && UpVelocityMag != 0.f && IsMovingOnGround())
	{
		float LiftoffBound;
...
void UCharacterMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	...
	const float VelocityUp = Velocity | -GravDir;
	float OriginalAccelUp = Acceleration | -GravDir;
	bool bLimitedUpAccel = false;

	if (!HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity() && NetBuoyancy != 0.0f && VelocityUp > MaxSwimSpeed * 0.33f)
	{
		//damp positive Z out of water
		Velocity = FVector::VectorPlaneProject(Velocity, GravDir) - GravDir * FMath::Max(0.33f * MaxSwimSpeed, VelocityUp * Depth*Depth);
	}
	else if (Depth < 0.65f)
	{
		bLimitedUpAccel = OriginalAccelUp > 0.f;
		if (0.1f < OriginalAccelUp)
		{
			Acceleration = FVector::VectorPlaneProject(Acceleration, GravDir) - GravDir * 0.1f;
		}
	}
	...
	{
		const float velUp = Velocity | -GravDir;
		if (bLimitedUpAccel && (velUp >= 0.f))
		{
			// allow upward velocity at surface if against obstacle
			Velocity = FVector::VectorPlaneProject(Velocity, GravDir) - GravDir * (velUp + OriginalAccelUp * deltaTime);
		...
		if (UpDown < 0.5f && UpDown > -0.2f && FMath::Abs(Hit.ImpactNormal | GravDir) < 0.2f && CanStepUp(Hit))
		{
			...
			Velocity = FVector::VectorPlaneProject(Velocity, GravDir) - GravDir; // HACK: since will be moving up, in case pawn leaves the water.
			bSteppedUp = StepUp(GravDir, Adjusted * (1.f - Hit.Time), Hit);
			if (bSteppedUp)
			{
				//may have left water - if so, script might have set new physics mode
	...
	if( !HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity() && !bJustTeleported && ((deltaTime - remainingTime) > KINDA_SMALL_NUMBER) && CharacterOwner )
	{
		...
		if (bWaterJump)
		{
			Velocity = FVector::VectorPlaneProject(Velocity, GravDir) - GravDir * velUp;
		}
	}
...
FVector UCharacterMovementComponent::GetFallingLateralAcceleration(float DeltaTime, const FVector& GravDir)
{
	// No acceleration
...
void UCharacterMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	FVector FallAcceleration = GetFallingLateralAcceleration(deltaTime, GravityDir);
	...
	while( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) )
	{
		...
		if (bNotifyApex && CharacterOwner->Controller && ((Velocity | GravityDir) * -1.0f) <= 0.f)
		{
		...
		else if ( Hit.bBlockingHit )
		{
			...
			else
			{
				...
				if (bHasAirControl)
				{
					...
					const FVector AirControlDeltaV = LimitAirControl(LastMoveTimeSlice, AirControlAccel, Hit, GravityDir, bCheckLandingSpot) * LastMoveTimeSlice;
				...
				if (subTimeTickRemaining > KINDA_SMALL_NUMBER && (Delta | Adjusted) > 0.f)
				{
					...
					if (Hit.bBlockingHit)
					{
						...
						{
							const bool bCheckLandingSpot = false; // we already checked above.
							const FVector AirControlDeltaV = LimitAirControl(subTimeTickRemaining, AirControlAccel, Hit, GravityDir, bCheckLandingSpot) * subTimeTickRemaining;
						...
						else if (GetPerchRadiusThreshold() > 0.f && Hit.Time == 1.f && (OldHitImpactNormal | GravityDir) <= -GetWalkableFloorZ())
						{
...
bool UCharacterMovementComponent::FindAirControlImpact(float DeltaTime, float AdditionalTime, const FVector& FallVelocity, const FVector& FallAcceleration, const FVector& Gravity, FHitResult& OutHitResult)
{
	...
	if (!TestWalk.IsZero())
	{
		...
		if (GetWorld()->SweepSingleByChannel(OutHitResult, CapsuleLocation, CapsuleLocation + TestWalk, GetCapsuleRotation(), UpdatedComponent->GetCollisionObjectType(), CapsuleShape, CapsuleQuery, ResponseParam))
		{
			return true;
		}
	}
...
FVector UCharacterMovementComponent::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, const FVector& GravDir, bool bCheckForValidLandingSpot)
{
...
FVector UCharacterMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FVector& DeltaPlaneNormal, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
...
void UCharacterMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult)
{
	...
	FHitResult Hit(1.0f);
	FVector RampVector = ComputeGroundMovementDelta(Delta, CapsuleUp, CurrentFloor.HitResult, CurrentFloor.bLineTrace);
	...
	else if (Hit.IsValidBlockingHit())
	{
		...
		if ((Hit.Time > 0.f) && (Hit.Normal.Z > KINDA_SMALL_NUMBER) && IsWalkable(Hit))
		{
			...
			RampVector = ComputeGroundMovementDelta(Delta * InitialPercentRemaining, CapsuleUp, Hit, false);
	...
	while ( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || HasAnimRootMotion() || CurrentRootMotion.HasOverrideVelocity() || (CharacterOwner->Role == ROLE_SimulatedProxy)) )
	{
		...
		else
		{
			...
			if ( IsFalling() )
			{
				...
				if (DesiredDist > KINDA_SMALL_NUMBER)
				{
					const float ActualDist = FVector::VectorPlaneProject(UpdatedComponent->GetComponentLocation() - OldLocation, GetCapsuleAxisZ()).Size();
...
void UCharacterMovementComponent::PhysNavWalking(float deltaTime, int32 Iterations)
{
	...
	//bound acceleration
	Acceleration = FVector::VectorPlaneProject(Acceleration, GetCapsuleAxisZ());
	if (!HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity())
	{
	...
	DesiredMove = FVector::VectorPlaneProject(DesiredMove, GetCapsuleAxisZ());
...
void UCharacterMovementComponent::OnTeleported()
{
	...
	if (OldBase && CurrentFloor.IsWalkableFloor() && CurrentFloor.FloorDist <= MAX_FLOOR_DIST && (Velocity | GetCapsuleAxisZ()) <= 0.f)
	{
...
void UCharacterMovementComponent::PhysicsVolumeChanged( APhysicsVolume* NewVolume )
{
	...
	else if ( IsSwimming() )
	{
		...
		if ((Acceleration | GravityDir) < 0.0f && ShouldJumpOutOfWater(JumpDir, GravityDir) && (JumpDir | Acceleration) > 0.0f && CheckWaterJump(JumpDir, GravityDir, WallNormal))
		{
...
bool UCharacterMovementComponent::ShouldJumpOutOfWater(FVector& JumpDir, const FVector& GravDir)
{
...
bool UCharacterMovementComponent::CheckWaterJump(const FVector JumpDir, const FVector& GravDir, FVector& WallNormal)
{
	if (!HasValidData())
	{
		return false;
	...
	FVector CheckPoint = UpdatedComponent->GetComponentLocation() + FVector::VectorPlaneProject(JumpDir, GravDir).GetSafeNormal() * (PawnCapsuleRadius * 1.2f);
	...
	const ECollisionChannel CollisionChannel = UpdatedComponent->GetCollisionObjectType();
	bool bHit = GetWorld()->SweepSingleByChannel(HitInfo, UpdatedComponent->GetComponentLocation(), CheckPoint, GetCapsuleRotation(), CollisionChannel, CapsuleShape, CapsuleParams, ResponseParam);

	if ( bHit && !Cast<APawn>(HitInfo.GetActor()) )
	{
		// hit a wall - check if it is low enough
		WallNormal = -1.f * HitInfo.ImpactNormal;
		FVector Start = UpdatedComponent->GetComponentLocation() + GravDir * -MaxOutOfWaterStepHeight;
			...
			if (IsFlying())
			{
				if (CanStepUp(Hit))
				{
					...
					if (FMath::Abs(Hit.ImpactNormal | CapsuleDown) < 0.2f)
					{
						...
						{
							bSteppedUp = StepUp(CapsuleDown, Delta * (1.f - Hit.Time), Hit, OutStepDownResult);
						}
					}
				}
			}
...
bool UCharacterMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& CapsuleDown, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
...
bool UCharacterMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult &InHit, FStepDownResult* OutStepDownResult)
{
	...
	if (IsMovingOnGround() && CurrentFloor.IsWalkableFloor())
	{
		...
		const bool bHitVerticalFace = !IsWithinEdgeTolerance(InHit.Location, CapsuleDown, InHit.ImpactPoint, PawnRadius);
	...
	//if (InitialImpactZ <= PawnInitialFloorBase.Z)
	//{
	//	return false;
	//}
...
void UCharacterMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
	...
	T = FString::Printf(TEXT("%s In physicsvolume %s on base %s component %s gravity (%f, %f, %f)"), *GetMovementName(), (PhysicsVolume ? *PhysicsVolume->GetName() : TEXT("None")),
		(BaseActor ? *BaseActor->GetName() : TEXT("None")), (BaseComponent ? *BaseComponent->GetName() : TEXT("None")), GetGravity().X, GetGravity().Y, GetGravity().Z);
	DisplayDebugManager.DrawString(T);
}
...
bool UCharacterMovementComponent::MoveUpdatedComponentImpl(const FVector& Delta, const FQuat& NewRotation, bool bSweep, FHitResult* OutHit, ETeleportType Teleport)
{
	if (UpdatedComponent)
	{
		//		const FRotator& Rot = NewRotation.Rotator();
		//		const FQuat& Quat = ConstrainComponentRotation(Rot).Quaternion();
		return Super::MoveUpdatedComponentImpl(Delta, ConstrainComponentRotation(NewRotation.Rotator()).Quaternion(), bSweep, OutHit, Teleport);
	}
	
	return false;
}

FRotator UCharacterMovementComponent::ConstrainComponentRotation(const FRotator& Rotation) const
{
...
void UCharacterMovementComponent::UpdateComponentRotation()
{
	if (!UpdatedComponent)
	{
		return;
	...
	FVector CapsuleAxisX = GetCapsuleAxisX();
	
	//A step in MakeFromZX is broken so we do it ourselves
	if (FMath::Abs(DesiredCapsuleUp | CapsuleAxisX) >= THRESH_NORMALS_ARE_PARALLEL)
	{
		CapsuleAxisX = (FMath::Abs(DesiredCapsuleUp.Z) < (1.f - KINDA_SMALL_NUMBER)) ? FVector(0, 0, 1.f) : FVector(1.f, 0, 0);
	}
	
	// Take desired Z rotation axis of capsule, try to keep current X rotation axis of capsule.
	const FMatrix RotationMatrix = FRotationMatrix::MakeFromZX(DesiredCapsuleUp, CapsuleAxisX);
...
FORCEINLINE FQuat UCharacterMovementComponent::GetCapsuleRotation() const
{
	return UpdatedComponent->GetComponentQuat();
}

FORCEINLINE FVector UCharacterMovementComponent::GetCapsuleAxisX() const
{
...
FORCEINLINE FVector UCharacterMovementComponent::GetCapsuleAxisZ() const
{
...
FVector UCharacterMovementComponent::GetGravity() const
{
	...
	{
		return CustomGravity * GravityScale;
	}

	return Super::GetGravity() * GravityScale;
}

FVector UCharacterMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
	FVector CurrentGravity = GetGravity();
	...
	if (bAvoidZeroGravity)
	{
		if (CurrentGravity.IsZero())
		{
			if (bUseCustomGravity)
			{
				if (CustomGravity.IsZero())
				{
					if (GravityScale != 0)
					{
						return FVector(0.f, 0.f, -1.f) * ((GravityScale > 0.0f) ? 1.0f : -1.0f);
					}
					return FVector(0.f, 0.f, -1.f);
				}
				return CustomGravity.GetSafeNormal();
			}
			else
			{
				FVector WorldGrav = Super::GetGravity();
				if (WorldGrav.IsZero())
				{
					if (GravityScale != 0)
					{
				...
				return WorldGrav.GetSafeNormal();
			}
		}
	}

	return CurrentGravity.GetSafeNormal();
}

float UCharacterMovementComponent::GetGravityMagnitude() const
{
	if (bUseCustomGravity)
	{
		return CustomGravity.Size();
	}
	else
	{
		return Super::GetGravity().Size();
	}
}

void UCharacterMovementComponent::SetCustomGravity(FVector NewGravity)
{
